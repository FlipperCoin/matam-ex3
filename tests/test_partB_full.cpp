...
class OpenEventTextCase : ::testing::Test {
protected:
    class CanRegister {
    private:
        const std::vector<int>& students;
    public:
        explicit CanRegister(const std::vector<int>& students) : students(students) {}
        bool operator()(const int& s) {
            return std::find(students.begin(),students.end(),s) != students.end();
        }
    };

    DateWrap d1 = DateWrap(5,5,5);
    std::string oe_name = "oe1";
    OpenEvent *oe;
    std::string closede_name = "closede1";
    ClosedEvent *closede;
    std::string custe_name = "custe1";
    std::vector<int> allowed_students;
    CustomEvent<CanRegister> *custe;

    void SetUp() override {
        oe = new OpenEvent(d1, oe_name);
        closede = new ClosedEvent(d1, closede_name);
        allowed_students = {1,2,3};
        custe = new CustomEvent<CanRegister>(d1, custe_name, CanRegister(allowed_students));
    }
};
...
static void a() {
    ...
    auto d = c;
    auto e = c;
    e = SharedPointer<BaseEvent>(new OpenEvent(DateWrap(1,1,1),"lalala"));
    auto a = e;
    p = e;
    auto f = p;
    f = a;
